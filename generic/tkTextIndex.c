...
// from 8.6 on tkTextIndexType is defined as const
...
#ifdef STEXT_LINE_VISIBLE
	    if ((!checkElided || GetLineVisible(textPtr, dstPtr->linePtr))
		    && !elide) {
#else
	    if (!elide) {
#endif
...
#ifdef STEXT_LINE_VISIBLE
		if ((checkElided && !GetLineVisible(textPtr, linePtr1))
			|| elide) {
#else
		if (elide) {
#endif
...
#ifdef STEXT_DIFF /* BUG because a mark can be before -start */
	if (lineIndex <= 0) {
#else
	if (lineIndex == 0) {
#endif
