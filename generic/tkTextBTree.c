...
typedef struct Node {
    ...
    int *numLinesVisible;	/* Total number of non-hidden lines in
				 * the subtree rooted here, one entry for
				 * each peer widget. */
#endif
...
#define CSEG_SIZE(chars) ((unsigned) (Tk_Offset(TkTextSegment, body.chst.data) \
	+ (1 + (chars)) * 2))
    ...
    TkSharedText *sharedTextPtr)
{
    ...
    STEXT_INIT_LINE(linePtr)
    STEXT_INIT_LINE(linePtr2)
    ...
    linePtr->peerData = NULL;
    linePtr2->peerData = NULL;
#else
    linePtr->pixels = NULL;
    ...
    segPtr->size = 1;
#ifdef STEXT_STYLE_HACK
    segPtr->body.chst.chars = segPtr->body.chst.data;
    segPtr->body.chst.style = segPtr->body.chst.data + segPtr->size + 1;
    segPtr->body.chst.style[0] = -1;
    segPtr->body.chst.style[1] = -1;
#endif
    segPtr->body.chars[0] = '\n';
...
#ifdef STEXT_LINE_FLAGS
		linePtr->peerData = (TkTextLinePeerData *) ckrealloc(
			(char *) linePtr->peerData,
			sizeof(TkTextLinePeerData) * newPixelReferences);
#else
...
#ifdef STEXT_LINE_FLAGS
	    linePtr->peerData[useReference].pixels = (*counting ? defaultHeight : 0);
	    linePtr->peerData[useReference].epoch = (*counting ? 0 : 1);
	    pixelCount += linePtr->peerData[useReference].pixels;

	    /* Changing -start or -end loses info about line visibility
	     * and fold state. */
	    linePtr->peerData[useReference].flags = 0;
#else
...
#ifdef STEXT_LINE_FLAGS
	nodePtr->numLinesVisible = (int *) ckrealloc(
		(char *) nodePtr->numLinesVisible,
		sizeof(int) * newPixelReferences);
#endif
    }
    nodePtr->numPixels[useReference] = pixelCount;
#ifdef STEXT_LINE_FLAGS
    nodePtr->numLinesVisible[useReference] = nodePtr->numLines;
#endif
    return pixelCount;
}

				 ...
				 * with the last one. */
{
...
#ifdef STEXT_LINE_FLAGS
	nodePtr->numLinesVisible[overwriteWithLast] =
		nodePtr->numLinesVisible[treePtr->pixelReferences-1];
#endif
    }
    if (treePtr->pixelReferences == 1) {
	nodePtr->numPixels = NULL;
#ifdef STEXT_LINE_FLAGS
	nodePtr->numLinesVisible = NULL;
#endif
    } else {
	nodePtr->numPixels = (int *) ckrealloc((char *) nodePtr->numPixels,
	...
	while (linePtr != NULL) {
	    if (overwriteWithLast != -1) {
#ifdef STEXT_LINE_FLAGS
		linePtr->peerData[overwriteWithLast] =
			linePtr->peerData[treePtr->pixelReferences-1];
#else
		...
		linePtr->peerData = (TkTextLinePeerData *) ckrealloc(
			(char *) linePtr->peerData,
			sizeof(TkTextLinePeerData)
			* (treePtr->pixelReferences-1));
#else
		linePtr->pixels = (int *) ckrealloc((char *) linePtr->pixels,
...
#ifdef STEXT_LINE_FLAGS
    ckfree((char *) nodePtr->numLinesVisible);
#endif
    ckfree((char *) nodePtr);
}

...
#ifdef STEXT_LINE_FLAGS
    changeToPixelCount = newPixelHeight - linePtr->peerData[pixelReference].pixels;
#else
...
#ifdef STEXT_LINE_FLAGS
    linePtr->peerData[pixelReference].pixels = newPixelHeight;
#else
				 ...
				 * newlines, must be null-terminated). */
{
	...
	segPtr->body.chst.chars = segPtr->body.chst.data;
	segPtr->body.chst.style = segPtr->body.chst.data + segPtr->size + 1;
	memset(segPtr->body.chst.style, -1, segPtr->size + 1);
#endif
...
#ifdef STEXT_LINE_FLAGS
	newLinePtr->peerData = (TkTextLinePeerData *)
		ckalloc(sizeof(TkTextLinePeerData)*treePtr->pixelReferences);
#else
	...
	for (ref = 0; ref < treePtr->pixelReferences; ref++) {
#ifdef STEXT_LINE_FLAGS
	    newLinePtr->peerData[ref].pixels = linePtr->peerData[ref].pixels;
	    newLinePtr->peerData[ref].epoch = 0;
	    changeToPixelCount[ref] += newLinePtr->peerData[ref].pixels;
	    newLinePtr->peerData[ref].flags = 0;
#else
...
#ifdef STEXT_DIFF
    LexerInsertion(treePtr->sharedTextPtr,
	    TkBTreeLinesTo(NULL, indexPtr->linePtr),
	    changeToLineCount);
#endif
...
#if defined(STEXT_LINE_VISIBLE) && defined(STEXT_LINE_FLAGS)
	    /* New lines are visible by default. */
	    nodePtr->numLinesVisible[ref] += changeToLineCount;
#endif
	}
    }
    if (treePtr->pixelReferences > PIXEL_CLIENTS) {
	...
	for (peer = treePtr->sharedTextPtr->peers;
		peer != NULL;
		peer = peer->next) {
	    TkTextMarginLineCountChanged(peer, changeToLineCount);
	}
    }
#endif
}
				 ...
				 * that is to be deleted. */
{
	...
	if (segPtr == NULL) {
		...
		for (nodePtr = curNodePtr; nodePtr != NULL;
		    ...
		    for (ref = 0; ref < treePtr->pixelReferences; ref++) {
#ifdef STEXT_LINE_FLAGS
			nodePtr->numPixels[ref] -= curLinePtr->peerData[ref].pixels;
#else
			nodePtr->numPixels[ref] -= curLinePtr->pixels[2*ref];
#endif
#if defined(STEXT_LINE_VISIBLE) && defined(STEXT_LINE_FLAGS)
			if (!(curLinePtr->peerData[ref].flags & LINE_FLAG_HIDDEN) /* GetLineVisible(NULL, curLinePtr) */)
			    nodePtr->numLinesVisible[ref]--;
#endif
		    }
		}
		changeToLineCount++;
		...
		if (curLinePtr->flags & LINE_FLAG_MARKER1TO4) {
		    TkTextLineMarkerLineDeleted(treePtr->sharedTextPtr,
			    curLinePtr);
		}
#endif
#ifdef STEXT_LINE_FLAGS
		ckfree((char *) curLinePtr->peerData);
#else
	...
	for (nodePtr = curNodePtr; nodePtr != NULL;
	    ...
	    for (ref = 0; ref < treePtr->pixelReferences; ref++) {
#ifdef STEXT_LINE_FLAGS
		nodePtr->numPixels[ref] -= index2Ptr->linePtr->peerData[ref].pixels;
#else
		nodePtr->numPixels[ref] -= index2Ptr->linePtr->pixels[2*ref];
#endif
#if defined(STEXT_LINE_VISIBLE) && defined(STEXT_LINE_FLAGS)
		if (!(index2Ptr->linePtr->peerData[ref].flags & LINE_FLAG_HIDDEN) /*GetLineVisible(NULL, index2Ptr->linePtr)*/)
		    nodePtr->numLinesVisible[ref]--;
#endif
	    }
	}
...
#ifdef STEXT_LINE_MARKERS
	if (index2Ptr->linePtr->flags & LINE_FLAG_MARKER1TO4) {
	    TkTextLineMarkerLineDeleted(treePtr->sharedTextPtr,
		    index2Ptr->linePtr);
	}
#endif
#ifdef STEXT_LINE_FLAGS
	ckfree((char *) index2Ptr->linePtr->peerData);
#else
...
#ifdef STEXT_DIFF
    LexerDeletion(treePtr->sharedTextPtr,
...
#ifdef STEXT_LINE_FLAGS
	    linePtr->peerData[pixelReference].pixels < pixels;
#else
...
#ifdef STEXT_LINE_FLAGS
	pixels -= linePtr->peerData[pixelReference].pixels;
#else
...
#ifdef STEXT_LINE_FLAGS
	index += linePtr2->peerData[pixelReference].pixels;
#else
...
TkTextLine *
TkBTreeFindLineVisible(tree, textPtr, line)
    TkTextBTree tree;		/* B-tree in which to find line. */
    CONST TkText *textPtr;	/* Relative to this client of the B-tree */
    int line;			/* Index of desired line. */
{
    ...
    nodePtr = treePtr->rootPtr;
    linesLeft = line;
    if ((line < 0) || (line >= nodePtr->numLinesVisible[ref])) {
	return NULL;
    }
    ...
    while (nodePtr->level != 0) {
	for (nodePtr = nodePtr->children.nodePtr;
		nodePtr->numLinesVisible[ref] <= linesLeft;
		nodePtr = nodePtr->nextPtr) {
	    ASSERTM(nodePtr != NULL, "ran out of nodes");
	    linesLeft -= nodePtr->numLinesVisible[ref];
	}
    }
    ...
    for (linePtr = nodePtr->children.linePtr; linesLeft > 0;
	    linePtr = linePtr->nextPtr) {
	ASSERTM(linePtr != NULL, "ran out of lines");
	linesLeft -= GetLineVisible(textPtr, linePtr) ? 1 : 0;
    }
    return linePtr;
}

TkTextLine *
TkBTreeNextLineVisible(textPtr, linePtr)
    CONST TkText *textPtr;	/* Next line in the context of this client */
    register TkTextLine *linePtr;
{
    int ref = textPtr->pixelReference;
    register Node *nodePtr;
    register Node *parentPtr;
    register TkTextLine *nextPtr;

    /* Look for a following line in the same node. */
    nextPtr = linePtr->nextPtr;
    while (nextPtr != NULL) {
	if (GetLineVisible(textPtr, nextPtr))
	    return nextPtr;
	nextPtr = nextPtr->nextPtr;
    }

    /* Look at the nodes following each ancestor of the line. */
    for (parentPtr = linePtr->parentPtr; ; parentPtr = parentPtr->parentPtr) {
	for (nodePtr = parentPtr->nextPtr;
		nodePtr != NULL;
		nodePtr = nodePtr->nextPtr) {
	    if (nodePtr->numLinesVisible[ref] > 0)
		break;
	}
	if (nodePtr != NULL)
	    break;
    ...
    ASSERT(nodePtr != NULL && nodePtr->numLinesVisible[ref] > 0);

    /* Walk down to level 0. */
    while (nodePtr->level > 0) {
	nodePtr = nodePtr->children.nodePtr;
	while (nodePtr->numLinesVisible[ref] <= 0)
	    nodePtr = nodePtr->nextPtr;
    }

    ASSERT(nodePtr != NULL && nodePtr->level == 0 && nodePtr->numLinesVisible[ref] > 0);

    /* Finally, search level 0 for its first visible line. */
    for (linePtr = nodePtr->children.linePtr;
	    linePtr != NULL;
	    linePtr = linePtr->nextPtr) {
    ...
    ASSERTM(0, "no visible line");
    return NULL;
}

TkTextLine *
TkBTreePreviousLineVisible(textPtr, linePtr)
    CONST TkText *textPtr;	/* Next line in the context of this client */
    register TkTextLine *linePtr;
{
    int ref = textPtr->pixelReference;
    ...
    Node *nodePrevPtr;
    register TkTextLine *prevPtr;
    TkTextLine *prevVisPtr;

    /* Check for a visible line under the same parent. */
    if (linePtr->parentPtr->numLinesVisible[ref] > 0) {
	prevPtr = linePtr->parentPtr->children.linePtr;	/* First line at leaf */
	prevVisPtr = NULL;
	while (prevPtr != linePtr) {
	    if (GetLineVisible(textPtr, prevPtr))
		prevVisPtr = prevPtr;
	    if (prevPtr->nextPtr == linePtr) {
		break;
	    }
    ...
    /* Find an ancestor with a previous sibling node that has some visible
     * lines. These lines are before the line we care about. */
    nodePrevPtr = NULL;
    for (nodePtr = linePtr->parentPtr; ; nodePtr = nodePtr->parentPtr) {
	...
	if (nodePtr != nodePtr->parentPtr->children.nodePtr) {
	    for (node2Ptr = nodePtr->parentPtr->children.nodePtr;
		    node2Ptr != nodePtr;
		    node2Ptr = node2Ptr->nextPtr) {
		if (node2Ptr->numLinesVisible[ref] > 0)
		    nodePrevPtr = node2Ptr;
	    }
	    if (nodePrevPtr != NULL)
		break;
	}
    ...
    if (nodePrevPtr == NULL) {
dbwin("TkBTreePreviousLineVisible: no visible nodes before line %d\n", BTREE_LINESTO(textPtr, linePtr) + 1); 
	return NULL;
    }

    /* Now we have a node that is the ancestor of a visible line that comes
     * before the line we care about. Search down to level 0. */
    if (nodePrevPtr->level > 0) {
	nodePtr = nodePrevPtr;
	nodePrevPtr = NULL;
	for (node2Ptr = nodePtr->children.nodePtr; ;
	    ...
	    node2Ptr = nodePrevPtr;
	    nodePrevPtr = NULL;
	}
    }
    ASSERT(nodePrevPtr != NULL && nodePrevPtr->level == 0);
    node2Ptr = nodePrevPtr;

    /* Finally, search the level 0 node for its last visible line. */
    prevVisPtr = NULL;
    for (prevPtr = node2Ptr->children.linePtr; ; prevPtr = prevPtr->nextPtr) {
	if (GetLineVisible(textPtr, prevPtr))
	    prevVisPtr = prevPtr;
    ...
    ASSERT(prevVisPtr != NULL);
    return prevVisPtr;
}

int
TkBTreeLinesToVisible(textPtr, linePtr)
    CONST TkText *textPtr;	/* Relative to this client of the B-tree */
    TkTextLine *linePtr;	/* Pointer to existing line in B-tree. */
{
    int ref = textPtr->pixelReference;
    ...
    /*
     * First count how many visible lines precede this one in its level-0
     * node.
     */
    ...
    for (linePtr2 = nodePtr->children.linePtr; linePtr2 != linePtr;
	    linePtr2 = linePtr2->nextPtr) {
	ASSERTM(linePtr2 != NULL, "couldn't find line");
	if (GetLineVisible(textPtr, linePtr2))
	    index += 1;
    }

    /*
     * Now work up through the levels of the tree one at a time,
     * counting how many visible lines are in nodes preceding the current
     * node.
     */
	...
	for (nodePtr2 = parentPtr->children.nodePtr; nodePtr2 != nodePtr;
		nodePtr2 = nodePtr2->nextPtr) {
	    ASSERTM(nodePtr2 != NULL, "couldn't find node");
	    index += nodePtr2->numLinesVisible[ref];
	}
    }

    if (textPtr->start != NULL && linePtr != textPtr->start) {
	index -= TkBTreeLinesToVisible(textPtr, textPtr->start);
    }
...
TkBTreeToggleLineVisible(textPtr, linePtr)
    CONST TkText *textPtr;	/* In the context of this client. */
    TkTextLine *linePtr;		/* Pointer to existing line in
					 * B-tree. */
{
    int ref = textPtr->pixelReference;
    bool visible = GetLineVisible(textPtr, linePtr);
    register Node *nodePtr, *parentPtr;

    nodePtr = linePtr->parentPtr;
    nodePtr->numLinesVisible[ref] += visible ? 1 : -1;

    for (parentPtr = nodePtr->parentPtr; parentPtr != NULL;
	    parentPtr = parentPtr->parentPtr) {
	parentPtr->numLinesVisible[ref] += visible ? 1 : -1;
    }
}
...
TkBTreeGetTags(
    ...
    const TkText *textPtr,	/* If non-NULL, then only return tags for this
				 ...
				 * widgets). */
#ifdef STEXT_DIFF
    TkTextTagInfo *tagInfo)	/* NULL or a pointer to a structure in which
				 * indexPtr's tags will be stored and
				 * returned. */
#else
...
#endif
{
    ...
    tagInfo->numTags = 0;
    tagInfo->arraySize = LOTSA_TAGS;
    tagInfo->counts = tagInfo->deftagCnts;
    tagInfo->tagPtrs = tagInfo->deftagPtrs;
#else
    TagInfo tagInfo;
#define NUM_TAG_INFOS 10
...
#ifdef STEXT_STYLE_HACK
    if (textPtr != NULL) {
	TkTextTag *tagPtr = Lexer_TagAtIndex(textPtr->sharedTextPtr->lexer,
		indexPtr);

	if (tagPtr != NULL) {
	    IncCount(tagPtr, 1, tagInfo);
	}
    }
#endif /* STEXT_STYLE_HACK */
    ...
    tagInfo->numTags = dst;
    return (dst == 0) ? NULL : tagInfo->tagPtrs;
#else
    for (src = 0, dst = 0; src < tagInfo.numTags; src++) {
	if (tagInfo.counts[src] & 1) {
	    const TkText *tagTextPtr = tagInfo.tagPtrs[src]->textPtr;
...
TkTextFreeTagInfo(tagInfo)
    TkTextTagInfo *tagInfo; /* Free any allocated memory in this
				 * structure. */
{
    if (LOTSA_TAGS < tagInfo->arraySize) {
	ckfree((char*)tagInfo->counts);
	ckfree((char*)tagInfo->tagPtrs);
    }
}
...
#ifdef STEXT_DIFF
    TkTextElideInfo elideInfoOnTheStack;
#endif
    TkTextLine *linePtr;
    int elide;
...
#ifdef STEXT_DIFF
    TkTextTagInfo *tagInfoPtr)	/* Holds cumulative information about tags;
...
#ifdef STEXT_DIFF
	if (tagInfoPtr->tagPtrs != tagInfoPtr->deftagPtrs)
#endif
...
#ifdef STEXT_DIFF
	if (tagInfoPtr->counts != tagInfoPtr->deftagCnts)
#endif
				 ...
				 * pixel counts. */
{
...
#ifdef STEXT_LINE_VISIBLE
    int *numLinesVisible;
    int linesVisible[PIXEL_CLIENTS];
#endif
    int *numPixels;
    int pixels[PIXEL_CLIENTS];
...
#ifdef STEXT_LINE_VISIBLE
	numLinesVisible = (int *) ckalloc(sizeof(int) * references);
#endif
    } else {
	numPixels = pixels;
#ifdef STEXT_LINE_VISIBLE
	numLinesVisible = linesVisible;
#endif
    }
	...
	for (linePtr = nodePtr->children.linePtr; linePtr != NULL;
	    ...
	    for (i = 0; i<references; i++) {
#ifdef STEXT_LINE_FLAGS
		numPixels[i] += linePtr->peerData[i].pixels;
#else
		numPixels[i] += linePtr->pixels[2 * i];
#endif
#ifdef STEXT_LINE_VISIBLE
		if (!(linePtr->peerData[i].flags & LINE_FLAG_HIDDEN) /*GetLineVisible(NULL, linePtr)*/)
		    numLinesVisible[i]++;
#endif
	    }
	}
...
#ifdef STEXT_LINE_VISIBLE
		numLinesVisible[i] += childNodePtr->numLinesVisible[i];
#endif
	    }
	}
    }
...
#ifdef STEXT_LINE_VISIBLE
	if (numLinesVisible[i] != nodePtr->numLinesVisible[i]) {
	    Tcl_Panic("CheckNodeConsistency: mismatch in numLinesVisible (%d %d)",
		    numLinesVisible[i], nodePtr->numLinesVisible[i]);
	}
#endif
    }
    if (references > PIXEL_CLIENTS) {
	...
	if (nodePtr->numChildren > MAX_CHILDREN) {
		...
		if (nodePtr->parentPtr == NULL) {
		    ...
		    newPtr->numPixels = (int *)
			    ckalloc(sizeof(int) * treePtr->pixelReferences);
#ifdef STEXT_LINE_VISIBLE
		    newPtr->numLinesVisible = (int *)
			    ckalloc(sizeof(int) * treePtr->pixelReferences);
#endif
		    for (i=0; i<treePtr->pixelReferences; i++) {
			newPtr->numPixels[i] = nodePtr->numPixels[i];
#ifdef STEXT_LINE_VISIBLE
			newPtr->numLinesVisible[i] = nodePtr->numLinesVisible[i];
#endif
		    }
				 ...
				 * recomputed. */
{
	...
	for (linePtr = nodePtr->children.linePtr; linePtr != NULL;
	    ...
	    for (ref = 0; ref<treePtr->pixelReferences; ref++) {
#ifdef STEXT_LINE_FLAGS
		nodePtr->numPixels[ref] += linePtr->peerData[ref].pixels;
#else
		nodePtr->numPixels[ref] += linePtr->pixels[2 * ref];
#endif
#if defined(STEXT_LINE_VISIBLE) && defined(STEXT_LINE_FLAGS)
		if (!(linePtr->peerData[ref].flags & LINE_FLAG_HIDDEN) /*GetLineVisible(NULL, linePtr)*/)
		    nodePtr->numLinesVisible[ref]++;
#endif
	    }
...
#ifdef STEXT_LINE_VISIBLE
		nodePtr->numLinesVisible[ref] += childPtr->numLinesVisible[ref];
#endif
	    }
...
#ifdef STEXT_LINE_VISIBLE
int
TkBTreeNumLinesVisible(tree, textPtr)
    TkTextBTree tree;		/* Information about tree. */
    CONST TkText *textPtr;	/* Relative to this client of the B-tree */
{
    BTree *treePtr = (BTree *) tree;
    ...
    if (textPtr->end != NULL) {
	count = PEER_LINESTOVIS(textPtr, textPtr->end);
    } else {
	count = treePtr->rootPtr->numLinesVisible[textPtr->pixelReference];
    }
    if (textPtr->start != NULL) {
	count -= PEER_LINESTOVIS(textPtr, textPtr->start);
    }
				 ...
				 * split. */
{
    ...
    newPtr1->size = index;
#ifdef STEXT_STYLE_HACK
    newPtr1->body.chst.chars = newPtr1->body.chst.data;
    newPtr1->body.chst.style = newPtr1->body.chst.data + newPtr1->size + 1;
    memcpy(newPtr1->body.chst.style, segPtr->body.chst.style, (size_t) index);
    newPtr1->body.chst.style[index] = -1;
#endif
...
#ifdef STEXT_STYLE_HACK
    newPtr2->body.chst.chars = newPtr2->body.chst.data;
    newPtr2->body.chst.style = newPtr2->body.chst.data + newPtr2->size + 1;
    memcpy(newPtr2->body.chst.style, segPtr->body.chst.style + index,
	newPtr2->size);
#endif
...
#ifdef STEXT_STYLE_HACK
    newPtr->body.chst.chars = newPtr->body.chst.data;
    newPtr->body.chst.style = newPtr->body.chst.data + newPtr->size + 1;
    memcpy(newPtr->body.chst.style, segPtr->body.chst.style, segPtr->size);
    memcpy(newPtr->body.chst.style + segPtr->size, segPtr2->body.chst.style,
	segPtr2->size);
#endif
