...
static char *lineNumberStrings[] = {
    "document", "relative", NULL
};
#endif
 ...
 */
#ifndef DEF_TEXT_BLOCK_CURSOR
#define DEF_TEXT_BLOCK_CURSOR "0"
#define DEF_TEXT_INACTIVE_SELECT_COLOR NULL
#define DEF_TEXT_TABSTYLE "tabular"
#endif

/* Hack -- Don't call TkTextRelayoutWindow when only -cursor changed. */
#define TEXT_CFG_CURSOR 0x1000

/* Hack -- Don't call TkTextRelayoutWindow when only -autoseparators changed. */
#define TEXT_CFG_AUTOSEP 0x2000
...
static const Tk_OptionSpec optionSpecs[] = {
...
#ifdef STEXT_DIFF
	Tk_Offset(TkText, autoSeparators), 0, 0, TEXT_CFG_AUTOSEP},
#else
...
#ifdef STEXT_DIFF
	TK_OPTION_NULL_OK, 0, TEXT_CFG_CURSOR},
#else
	TK_OPTION_NULL_OK, 0, 0},
#endif
#ifdef STEXT_EDGE_LINE
    {TK_OPTION_INT, "-edgecolumn", (char *) NULL, (char *) NULL,
	"0", -1, Tk_Offset(TkText, edgeColumn), 0, NULL, 0},
    {TK_OPTION_COLOR, "-edgecolor", (char *) NULL, (char *) NULL,
	"gray", -1, Tk_Offset(TkText, edgeColorPtr), 0, NULL, 0},
#endif
	...
	0, 0, 0},
#ifdef STEXT_LINE_NUMBER
    {TK_OPTION_STRING_TABLE, "-linenumbers", NULL, NULL,
	"document", -1, Tk_Offset(TkText, lineNumbers), 0,
	(ClientData) lineNumberStrings, 0},
#endif
    ...
    {TK_OPTION_BOOLEAN, "-showeol", NULL, NULL,
	"0", -1, Tk_Offset(TkText, showEOL), 0, NULL, 0},
#endif
#ifdef STEXT_DRAW_TAB
    {TK_OPTION_BOOLEAN, "-showtabs", NULL, NULL,
	"0", -1, Tk_Offset(TkText, showTabs), 0, NULL, 0},
#endif
...
#ifdef STEXT_DIFF
    {TK_OPTION_INT, "-tabwidth", "tabWidth", "TabWidth",
	"8", -1, Tk_Offset(TkText, tabWidth), 0, NULL, 0},
#endif
...
#ifdef STEXT_DIFF
    {TK_OPTION_COLOR, "-whitespaceforeground", (char *) NULL, (char *) NULL,
	"gray", -1, Tk_Offset(TkText, wsColorPtr), 0, NULL, 0},
#endif
...
#ifdef STEXT_DIFF
    Tk_SetClass(textPtr->tkwin, "TextPlus");
#else
...
#ifdef STEXT_MARGINS
    TkTextInitMargins(textPtr);
#endif
#ifdef STEXT_DIFF
    if (ConfigureText(interp, textPtr, objc-2, objv+2, 1) != TCL_OK) {
#else
    ...
    Tcl_Obj *CONST objv[])	/* Argument objects. */
{
	...
	"delete", "dlineinfo", "dump", "edit", "get", "identify",
	"image", "index", "insert", "lexer", "linefoldable", "linefolded",
	"linefoldhighlight", "linefoldlevel", "linemarker", "linevisible",
	"margin", "mark", "peer", "replace", "scan", "search", "see", "tag",
	"togglecontraction", "window", "xview", "yview", NULL
    };
    enum options {
	TEXT_BBOX, TEXT_CGET, TEXT_COMPARE, TEXT_CONFIGURE, TEXT_COUNT,
	TEXT_DEBUG, TEXT_DELETE, TEXT_DLINEINFO, TEXT_DUMP, TEXT_EDIT,
	TEXT_GET, TEXT_IDENTIFY, TEXT_IMAGE, TEXT_INDEX, TEXT_INSERT,
	TEXT_LEXER, TEXT_LINEFOLDABLE, TEXT_LINEFOLDED, TEXT_LINEFOLDHIGHLIGHT,
	TEXT_LINEFOLDLEVEL, TEXT_LINEMARKER, TEXT_LINEVISIBLE, TEXT_MARGIN,
	TEXT_MARK, TEXT_PEER, TEXT_REPLACE, TEXT_SCAN, TEXT_SEARCH, TEXT_SEE,
	TEXT_TAG, TEXT_TOGGLECONTRACTION, TEXT_WINDOW, TEXT_XVIEW, TEXT_YVIEW
    };
#else
    static CONST char *optionStrings[] = {
...
#ifdef STEXT_DIFF
	    result = ConfigureText(interp, textPtr, objc-2, objv+2, 0);
#else
	...
	for (peer = textPtr->sharedTextPtr->peers;
		peer != NULL; peer = peer->next) {
	    SetFoldHighlightLine(peer, NULL);
#ifdef STEXT_MARGINS
	    TkTextMarginDeletion(peer);
#endif
	}
#endif
	...
	if (objc != 4) {
	    Tcl_WrongNumArgs(interp, 2, objv, "identify x y");
	    result = TCL_ERROR;
	    goto done;
	}
	if ((result = Tcl_GetIntFromObj(interp, objv[2], &x)) != TCL_OK)
	    goto done;
	if ((result = Tcl_GetIntFromObj(interp, objv[3], &y)) != TCL_OK)
	    goto done;
	TkTextIdentify(textPtr, x, y);
	break;
    }
#endif /* STEXT_DIFF */
    case TEXT_IMAGE:
...
#ifdef STEXT_FOLDING
    case TEXT_LINEFOLDABLE: {
	CONST TkTextIndex *indexPtr;
	TkTextLine *linePtr;

	if (objc != 3) {
	    Tcl_WrongNumArgs(interp, 2, objv, "linefoldable index");
	    result = TCL_ERROR;
	    goto done;
	}
	indexPtr = TkTextGetIndexFromObj(interp, textPtr, objv[2]);
	...
	/* FIXME: if no lexer or not enabled ... */
	/* FIXME: if LEX_NEEDED ... */
	Tcl_SetObjResult(interp, Tcl_NewBooleanObj(
		GetLineFoldable(textPtr->sharedTextPtr, linePtr)));
	break;
    }
    case TEXT_LINEFOLDED: {
	CONST TkTextIndex *indexPtr;
	TkTextLine *linePtr;
	int folded;

	if (objc < 3 || objc > 4) {
	    Tcl_WrongNumArgs(interp, 2, objv, "linefolded index ?boolean?");
	    result = TCL_ERROR;
	    goto done;
	}
	indexPtr = TkTextGetIndexFromObj(interp, textPtr, objv[2]);
	...
	if (objc == 3) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(
		    GetLineFolded(textPtr, linePtr)));
	} else {
	    if (Tcl_GetBooleanFromObj(interp, objv[3], &folded) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    }
	    folded = SetLineFolded(textPtr, linePtr, folded);
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(folded));
	}
	break;
    }
    case TEXT_LINEFOLDHIGHLIGHT: {
	CONST TkTextIndex *indexPtr;

	if (objc < 2 || objc > 3) {
	    Tcl_WrongNumArgs(interp, 2, objv, "linefoldhighlight ?index?");
	    result = TCL_ERROR;
	    goto done;
	}
	if (objc == 3) {
		...
		SetFoldHighlightLine(textPtr, indexPtr->linePtr);
	    } else {
		SetFoldHighlightLine(textPtr, NULL);
	    }
	}
	if (textPtr->foldHighlightLinePtr != NULL) {
	    char buf[TCL_INTEGER_SPACE];
	    sprintf(buf, "%d.0",
		PEER_LINESTO(textPtr, textPtr->foldHighlightLinePtr) + 1);
	    Tcl_SetResult(interp, buf, TCL_VOLATILE);
	}
	break;
    }
    case TEXT_LINEFOLDLEVEL: {
	CONST TkTextIndex *indexPtr;
	TkTextLine *linePtr;

	if (objc != 3) {
	    Tcl_WrongNumArgs(interp, 2, objv, "linefoldlevel index");
	    result = TCL_ERROR;
	    goto done;
	}
	indexPtr = TkTextGetIndexFromObj(interp, textPtr, objv[2]);
	...
	Tcl_SetObjResult(interp, Tcl_NewIntObj(
		GetLineFoldDepth(textPtr->sharedTextPtr, linePtr)));
	break;
    }
    case TEXT_TOGGLECONTRACTION: {
	CONST TkTextIndex *indexPtr;

	if (objc != 3) {
	    Tcl_WrongNumArgs(interp, 2, objv, "togglecontraction index");
	    result = TCL_ERROR;
	    goto done;
	}
	indexPtr = TkTextGetIndexFromObj(interp, textPtr, objv[2]);
	...
	/* FIXME: if no lexer or not enabled ... */
	/* FIXME: if LEX_NEEDED ... */
	ToggleContraction(textPtr, indexPtr->linePtr);
	break;
    }
#endif /* STEXT_FOLDING */
#ifdef STEXT_DIFF
    case TEXT_LEXER: {
	TkTextLexerCmd(textPtr, interp, objc, objv);
	break;
    }
#endif
#ifdef STEXT_LINE_MARKERS
    case TEXT_LINEMARKER:
	result = TkTextLineMarkerCmd(textPtr, interp, objc, objv);
	break;
#endif
#ifdef STEXT_LINE_VISIBLE
    case TEXT_LINEVISIBLE: {
	CONST TkTextIndex *indexPtr;
	TkTextLine *linePtr;
	int visible;

	if (objc < 3 || objc > 4) {
	    Tcl_WrongNumArgs(interp, 2, objv, "linevisible index ?boolean?");
	    result = TCL_ERROR;
	    goto done;
	}
	indexPtr = TkTextGetIndexFromObj(interp, textPtr, objv[2]);
	...
	if (objc == 3) {
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(
		    GetLineVisible(textPtr, linePtr)));
	} else {
	    if (Tcl_GetBooleanFromObj(interp, objv[3], &visible) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    }
	    if (visible != GetLineVisible(textPtr, linePtr)) {
		if (linePtr != BTREE_FINDLINE(textPtr, BTREE_NUMLINES(textPtr)))
		    visible = SetLineVisible(textPtr, linePtr, visible);
	    }
	    Tcl_SetObjResult(interp, Tcl_NewBooleanObj(visible));
	}
	break;
    }
...
#ifdef STEXT_MARGINS
    case TEXT_MARGIN:
	result = TkTextMarginCmd(textPtr, interp, objc, objv);
	break;
#endif
    case TEXT_MARK:
...
#ifdef STEXT_DIFF
    if (textPtr->wsGC != None) {
	Tk_FreeGC(Tk_Display(textPtr->tkwin), textPtr->wsGC);
    }
#endif
#ifdef STEXT_MARGINS
    TkTextFreeMargins(textPtr);
#endif
#ifdef STEXT_LINE_MARKERS
    TkTextFreeLineMarkers(textPtr);
#endif
...
#endif
{
	...
	/* If only -autoseparators changed, do not relayout the whole window. */
	if (mask & TEXT_CFG_AUTOSEP) {
	    textPtr->sharedTextPtr->autoSeparators = textPtr->autoSeparators;
	    return TCL_OK;
	}

	/* If only -cursor changed, do not relayout the whole window. */
	if (mask & TEXT_CFG_CURSOR)
	    return TCL_OK;
    }
#endif
...
#ifdef STEXT_DIFF
    /* Recreate the graphics context used for drawing whitespace. */
    if (textPtr->wsGC != None) {
	Tk_FreeGC(Tk_Display(textPtr->tkwin), textPtr->wsGC);
    }
    {
	...
	gcValues.foreground = textPtr->wsColorPtr->pixel;
	gcValues.line_width = 1;
	gcMask = GCForeground | GCLineWidth;
	textPtr->wsGC = Tk_GetGC(textPtr->tkwin, gcMask, &gcValues);
    }
#endif
    ...
    int viewUpdate)		/* Update vertical view if set. */
{
	...
	(void) TkBTreeGetTags(&index2, NULL, &tagInfo);
	for (i = 0; i < tagInfo.numTags; i++) {
	    TkBTreeTag(&index2, &oldIndex2, tagInfo.tagPtrs[i], 0);
	}
...
#ifdef STEXT_LINE_VISIBLE
		    && GetLineVisible(textPtr, textPtr->selIndex.linePtr)
...
TkpAlwaysShowSelection(tkwin)
    Tk_Window tkwin;			/* Window whose application is
					 * to be checked. */
{
#if defined(STEXT_DIFF) && TCL_MAJOR_VERSION == 8 && TCL_MINOR_VERISON < 5
    return 0;
#else
    ...
    int viewUpdate)		/* Update the view if set. */
{
...
#ifdef STEXT_DIFF
    for (peer = sharedTextPtr->peers; peer != NULL; peer = peer->next) {
	SetFoldHighlightLine(peer, NULL);
    }
#endif
	...
	if (objc > (j+1)) {
	    ...
	    (void) TkBTreeGetTags(&index1, NULL, &tagInfo);
	    for (i = 0; i < tagInfo.numTags; i++) {
		TkBTreeTag(&index1, &index2, tagInfo.tagPtrs[i], 0);
	    }
	    TkTextFreeTagInfo(&tagInfo);
#else
	    Tcl_Obj **tagNamePtrs;
...
#ifdef STEXT_LINE_VISIBLE
		|| (GetLineVisible(textPtr, curIndex.linePtr)
    ...
    Tcl_Obj *CONST objv[])	/* Argument objects. */
{
	...
	"canredo", "canundo", "modified", "redo", "reset", "separator", "undo", NULL
    };
    enum editOptions {
	EDIT_CANREDO, EDIT_CANUNDO, EDIT_MODIFIED, EDIT_REDO, EDIT_RESET,
    ...
    case EDIT_CANREDO: {
	int canRedo = 0;
	TkUndoAtom *elem = textPtr->sharedTextPtr->undoStack->redoStack;
	while (elem) {
    ...
    case EDIT_CANUNDO: {
	int canUndo = 0;
	TkUndoAtom *elem = textPtr->sharedTextPtr->undoStack->undoStack;
	while (elem) {
	    if (elem->type != TK_UNDO_SEPARATOR) {
		canUndo = 1;
		break;
...
#ifdef STEXT_LINE_VISIBLE
		if (!visibleOnly ||
		    (GetLineVisible(textPtr, tmpIndex.linePtr)
...
/*
 * In order to find stext.tcl during initialization, the following script
 * is invoked.
 */
static char initScript[] = "if {![llength [info proc ::TextPlus::Init]]} {\n\
  namespace eval ::TextPlus {}\n\
  proc ::TextPlus::Init {} {\n\
    global textplus_library\n\
    tcl_findLibrary TkTextPlus " PACKAGE_PATCHLEVEL " " PACKAGE_PATCHLEVEL " textplus.tcl TEXTPLUS_LIBRARY textplus_library\n\
  }\n\
}\n\
::TextPlus::Init";
    ...
    Tcl_Interp *interp)
{
    ...
    Tcl_CreateObjCommand(interp, "textplus", Tk_TextObjCmd, Tk_MainWindow(interp),
	NULL);
