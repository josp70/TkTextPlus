...
#ifndef TK_ISOLATE_END
/*
 * Define some stuff so compilation under 8.4 works.
 */
...
#ifdef STEXT_DLINE_CACHE
    Tcl_DString dString;	/* Holds all the characters for all the
				 * TkTextDispChunks on this display line. */
#endif
#ifdef STEXT_LINE_NUMBER
    int lineNumber;
#endif
#ifdef STEXT_FOLDING
    int foldMark;
...
#ifdef STEXT_FOLDING
#define DLINE_FOLDED	0x20	/* The logical line is folded. */
#define DLINE_FOLD_HIGHLIGHT 0x40 /* Set when the fold mark of the
				  * associated TkTextLine is drawn
				  * with the active color. */
#define DLINE_MARGINS	0x80	/* Set when margins need redrawing. */
...
typedef struct TextDInfo {
...
#ifdef STEXT_DLINE_CACHE
    DLine *dLineFreePtr;		/* Available records. */
    TkTextDispChunk *dChunkFreePtr;	/* Available records. */
    struct CharInfo *dCharInfoFreePtr;	/* Available records. */
#endif
    ...
    Tcl_TimerToken scrollbarTimer;
				 ...
				 * update callback. */
#ifdef STEXT_DIFF
    int insertLinesTo;		/* Used to track changes to the position of */
    int insertByteIndex;	/* the "insert" mark. */
#endif
} TextDInfo;
...
#ifdef STEXT_DRAW_EOL
    int newline;		/* Should an EOL indicator be drawn. */
#endif
    int numBytes;		/* Number of bytes to display. */
#ifdef STEXT_DLINE_CACHE
    int byteOffset;		/* Offset into DLine.dString. */
    CONST char *chars;		/* Points into DLine.dString. */
    struct CharInfo *nextPtr;	/* Next in list of free chunks. */
#else
...
#ifdef STEXT_DIFF
#define DINFO_UPDATE_METRICS	0x10
#define DINFO_RELAYOUT_WINDOW	0x20
#endif
...
#ifdef STEXT_DIFF
#define LayoutDLine(a,b) LayoutDLineXXX(__FILE__,__LINE__,a,b)
static DLine *		LayoutDLineXXX(CONST char *func, int line, TkText *textPtr,
...
#ifdef STEXT_DIFF
static int		NextTabStop(TkText *textPtr, Tk_Font tkfont, int x, int tabOrigin);
#else
...
#ifdef STEXT_DLINE_CACHE
    dInfoPtr->dLineFreePtr = NULL;
    dInfoPtr->dChunkFreePtr = NULL;
    dInfoPtr->dCharInfoFreePtr = NULL;
#endif
...
#ifdef STEXT_DLINE_CACHE
    /* Must do this after FreeDLines. */
    {
int count = 0;
	DLine *dlPtr = dInfoPtr->dLineFreePtr;
	while (dlPtr != NULL) {
	    DLine *nextPtr = dlPtr->nextPtr;
	    Tcl_DStringFree(&dlPtr->dString);
	    ckfree((char *) dlPtr);
count++;
	    dlPtr = nextPtr;
	}
dbwin("TkTextFreeDInfo: freed %d DLines", count);
    }
    {
int count = 0;
	TkTextDispChunk *chunkPtr = dInfoPtr->dChunkFreePtr;
	while (chunkPtr != NULL) {
...
dbwin("TkTextFreeDInfo: freed %d TkTextDispChunk", count);
    }
    {
int count = 0;
	CharInfo *ciPtr = dInfoPtr->dCharInfoFreePtr;
	while (ciPtr != NULL) {
	    CharInfo *nextPtr = ciPtr->nextPtr;
	    ckfree((char *) ciPtr);
count++;
	    ciPtr = nextPtr;
	}
dbwin("TkTextFreeDInfo: freed %d CharInfo", count);
    }
#endif
...
#ifdef STEXT_DIFF
    tagPtrs = TkBTreeGetTags(indexPtr, textPtr, &tagInfo);
    numTags = tagInfo.numTags;
#else
...
#ifdef STEXT_DLINE_CACHE
    char *oldDStringValue;
#endif
#ifdef STEXT_STYLE_HACK
    int styleIndex = -1;

    segPtr = TkTextIndexToSeg(indexPtr, &byteOffset);
    if (segPtr->typePtr == &tkTextCharType) {
	styleIndex = segPtr->body.chst.style[byteOffset];
    }
#endif
...
#ifdef STEXT_DLINE_CACHE
    if (textPtr->dInfoPtr->dLineFreePtr != NULL) {
	dlPtr = textPtr->dInfoPtr->dLineFreePtr;
	textPtr->dInfoPtr->dLineFreePtr = dlPtr->nextPtr;
	Tcl_DStringSetLength(&dlPtr->dString, 0);
    } else {
	dlPtr = (DLine *) ckalloc(sizeof(DLine));
	Tcl_DStringInit(&dlPtr->dString);
    }
#else
    dlPtr = (DLine *) ckalloc(sizeof(DLine));
#endif
...
#ifdef STEXT_LINE_VISIBLE
    /* If the line is not visible, return a DLine of zero size. */
    if (!GetLineVisible(textPtr, indexPtr->linePtr)) {
#if 0
	TkTextLine *linePtr;
	TkTextLine *fakeLinePtr = BTREE_FINDLINE(textPtr,
		BTREE_NUMLINES(textPtr));
#endif
	maxBytes = 0;
	for (segPtr = indexPtr->linePtr->segPtr; segPtr != NULL; segPtr = segPtr->nextPtr) {
	    maxBytes += segPtr->size;
	}

	/* Update the pixel height of the line. */
...
#if 0
	/* Merge any following hidden lines into this display line. */
	linePtr = PEER_NEXTLINE(textPtr, indexPtr->linePtr);
	while ((linePtr != NULL) && !GetLineVisible(textPtr, linePtr)) {
	    ...
	    TkBTreeLinePixelEpoch(textPtr, linePtr)
		    = textPtr->dInfoPtr->lineMetricUpdateEpoch;
	    if (TkBTreeLinePixelCount(textPtr, linePtr) != 0) {
		TkBTreeAdjustPixelHeight(textPtr, linePtr, 0, 0);
	    }

	    dlPtr->logicalLinesMerged++;
	    linePtr = PEER_NEXTLINE(textPtr, linePtr);
	    /* Don't merge with the fake end line. */
	    if (linePtr == fakeLinePtr)
		break;
	}
dbwin("LayoutDLine (%s:%d): merged lines %d-%d", file, line, PEER_LINESTO(textPtr, indexPtr->linePtr) + 1, PEER_LINESTO(textPtr, indexPtr->linePtr) + 1 + dlPtr->logicalLinesMerged);
#endif
	dlPtr->byteCount = maxBytes;
	dlPtr->spaceAbove = dlPtr->spaceBelow = dlPtr->length = 0;
	return dlPtr;
    }
#endif /* STEXT_LINE_VISIBLE */
  ...
  connectNextLogicalLine:
	...
	if (chunkPtr == NULL) {
#ifdef STEXT_DLINE_CACHE
	    if (textPtr->dInfoPtr->dChunkFreePtr != NULL) {
		chunkPtr = textPtr->dInfoPtr->dChunkFreePtr;
		textPtr->dInfoPtr->dChunkFreePtr = chunkPtr->nextPtr;
	    } else
#endif
...
#ifdef STEXT_STYLE_HACK
	/* The loop above breaks on tabs. This loop breaks on
	 * style changes also. */
	if (!elide && (segPtr->typePtr == &tkTextCharType)) {
	    char *p;
	    int i = byteOffset;
	    for (p = segPtr->body.chars + byteOffset; *p; p++, i++) {
		int thisStyle = segPtr->body.chst.style[i];
		if (thisStyle != styleIndex) {
		    maxBytes = i - byteOffset;
		    if (!maxBytes) {
			maxBytes = 1;
			if (*p != '\t')
			    gotTab = 0;
		    ...
		    styleIndex = thisStyle;
		    break;
		}
		/* Break on a tab before any style change */
		if (*p == '\t')
		    break;
	    }
	}
#endif /* STEXT_STYLE_HACK */
	chunkPtr->x = x;
	if (elide /*&& maxBytes*/) {
	    /*
...
#ifdef STEXT_DLINE_CACHE
	    oldDStringValue = Tcl_DStringValue(&dlPtr->dString);
	    if (segPtr->typePtr == &tkTextCharType) {
		/* layoutProc will overwrite this. */
		chunkPtr->clientData = &dlPtr->dString;
	    }
#endif
	    code = (*segPtr->typePtr->layoutProc)(textPtr, &curIndex, segPtr,
		    ...
		    chunkPtr);
#ifdef STEXT_DLINE_CACHE
	    if (oldDStringValue != Tcl_DStringValue(&dlPtr->dString)) {
		TkTextDispChunk *chunk2Ptr = dlPtr->chunkPtr;
		while (chunk2Ptr != NULL) {
		    if (chunk2Ptr->displayProc == CharDisplayProc) {
			CharInfo *ciPtr = chunk2Ptr->clientData;
			ciPtr->chars = Tcl_DStringValue(&dlPtr->dString) +
			    ciPtr->byteOffset;
		    }
		    chunk2Ptr = chunk2Ptr->nextPtr;
		}
	    }
#endif
	}
	if (code <= 0) {
	    ...
	    if (chunkPtr != NULL) {
#ifdef STEXT_DLINE_CACHE
		chunkPtr->nextPtr = textPtr->dInfoPtr->dChunkFreePtr;
		textPtr->dInfoPtr->dChunkFreePtr = chunkPtr;
#else
		ckfree((char *) chunkPtr);
#endif
	    }
	...
	if (breakByteOffset != breakChunkPtr->numBytes) {
	    ...
	    if (segPtr->typePtr == &tkTextCharType) {
		/* layoutProc will overwrite this. */
		breakChunkPtr->clientData = &dlPtr->dString;
	    }
#endif
...
#ifdef STEXT_FOLD_EXTRA_SPACE
    /* Leave space below for drawing the "folded" line, plus a 1-pixel gap */
    if (GetLineFolded(textPtr, dlPtr->index.linePtr)) {
	dlPtr->height += 2;
    }
#endif
    ...
    dlPtr->length = lastChunkPtr->x + lastChunkPtr->width;
#ifdef STEXT_EDGE_LINE
    if (dlPtr->length < textPtr->charWidth * textPtr->edgeColumn)
	dlPtr->length = textPtr->charWidth * textPtr->edgeColumn;
#endif
#ifdef STEXT_MARGINS
    if (dlPtr->index.byteIndex == 0) {
	dlPtr->flags |= DLINE_FIRST;
    }
    if (wholeLine) {
	dlPtr->flags |= DLINE_LAST;
    }
#endif
...
{
    TkTextLine *prevPtr, *nextPtr, *nearestPtr = NULL;
    int distancePrev = 0, distanceNext = 0;

    /* If the line itself is visible, then use it. */
    if (GetLineVisible(textPtr, linePtr)) {
	return linePtr;
    }

    /* Find a previous visible line. */
    prevPtr = TkBTreePreviousLineVisible(textPtr, linePtr);
    if (prevPtr != NULL) {
	distancePrev = PEER_LINESTO(textPtr, linePtr) - PEER_LINESTO(textPtr, prevPtr);
    }

    /* Find a following visible line. */
    nextPtr = TkBTreeNextLineVisible(textPtr, linePtr);
    if (nextPtr != NULL) {
	distanceNext = PEER_LINESTO(textPtr, linePtr) - PEER_LINESTO(textPtr, prevPtr);
    }
	...
	nearestPtr = nextPtr;
    } else if (distancePrev < distanceNext) {
	nearestPtr = prevPtr;
    } else {
	nearestPtr = nextPtr;
    }
    ...
    if (nearestPtr != NULL) {
dbwin("nearest visible line to line %d is %d\n", PEER_LINESTO(textPtr, linePtr) + 1, PEER_LINESTO(textPtr, nearestPtr) + 1);
	return nearestPtr;
    } else {
dbwin("nearest visible line to line %d is NONE\n", PEER_LINESTO(textPtr, linePtr) + 1);
    }

    /* NULL if all lines are hidden. */
    return nearestPtr;
}
#endif /* STEXT_LINE_VISIBLE */

#ifdef STEXT_FOLDING
static int
DLineDisplaysFoldLine(
    TkText *textPtr,
    DLine *dlPtr)
{
    return (dlPtr->flags & DLINE_LAST) &&
	GetLineFolded(textPtr, dlPtr->index.linePtr);
}
#endif

/*
...
#ifdef STEXT_DIFF
    if (dInfoPtr->flags & DINFO_RELAYOUT_WINDOW) {
	dInfoPtr->flags &= ~DINFO_RELAYOUT_WINDOW;
dbwin("DINFO_RELAYOUT_WINDOW %s", Tk_PathName(textPtr->tkwin));
	TkTextRelayoutWindow(textPtr, TK_TEXT_LINE_GEOMETRY);
    }

    /* This will call TkTextChanged on any affected lines, freeing
    * any DLines. */
    LexerLexNeeded(textPtr->sharedTextPtr);
#endif
...
#ifdef STEXT_FOLD_EXTRA_SPACE
	    /* A folded line gets 1 extra pixel of height for the dividing
	     * line. */
	    if (DLineDisplaysFoldLine(textPtr, dlPtr) !=
		    ((dlPtr->flags & DLINE_FOLDED) != 0)) {
		goto discardDLine;
	    }
#endif
...
#ifdef STEXT_LINE_NUMBER
	    /* If the line number changed, redraw the margins. */
	    if (!(dlPtr->flags & DLINE_MARGINS) &&
		    TkTextMarginBounds(textPtr, textPtr->marginN, NULL, NULL) &&
		    (dlPtr->lineNumber != TkBTreeLinesTo(textPtr, dlPtr->index.linePtr) + 1)) {
		dlPtr->flags |= DLINE_MARGINS;
	    }
#endif /* STEXT_LINE_NUMBER */
#ifdef STEXT_FOLDING
	    /* If the fold state changed, redraw the line. */
	    if (!(dlPtr->flags & OLD_Y_INVALID) &&
		    (((dlPtr->flags & DLINE_FOLDED) != 0) !=
		    DLineDisplaysFoldLine(textPtr, dlPtr))) {
		dlPtr->flags |= OLD_Y_INVALID;
	    }
	    /* If the fold mark or fold highlight changed,
	     * then redraw the margins. */
	    if (!(dlPtr->flags & DLINE_MARGINS) &&
		    TkTextMarginBounds(textPtr, textPtr->marginF, NULL, NULL) &&
		    ((dlPtr->foldMark != GetFoldMark(textPtr, dlPtr->index.linePtr, dlPtr->flags))
		    || (((dlPtr->flags & DLINE_FOLD_HIGHLIGHT) != 0) !=
			GetLineFoldHighlight(textPtr, dlPtr->index.linePtr)))) {
		dlPtr->flags |= DLINE_MARGINS;
	    }
#endif /* STEXT_FOLDING */
	...
	for (dlPtr = dInfoPtr->dLinePtr; dlPtr != NULL;
		dlPtr = dlPtr->nextPtr) {
#ifdef STEXT_MARGINS
	    dlPtr->flags |= OLD_Y_INVALID | DLINE_MARGINS;
#else
	    dlPtr->flags |= OLD_Y_INVALID;
#endif
	}
...
#ifdef STEXT_DLINE_CACHE
	firstPtr->nextPtr = textPtr->dInfoPtr->dLineFreePtr;
	textPtr->dInfoPtr->dLineFreePtr = firstPtr;
#else
...
#ifdef STEXT_EDGE_LINE
    if (textPtr->edgeColumn > 0) {
	GC gc;
	gc = Tk_GCForColor(textPtr->edgeColorPtr, Tk_WindowId(textPtr->tkwin));
	XFillRectangle(display, pixmap, gc,
	    dInfoPtr->x - dInfoPtr->curXPixelOffset +
		textPtr->charWidth * textPtr->edgeColumn, y,
	    (unsigned int) 1,
...
#ifdef STEXT_FOLDING
    /* Draw the divider line below this folded line if it is the last
     * display line for the associated logical line. */
    if (DLineDisplaysFoldLine(textPtr, dlPtr)) {
	GC gc;
	gc = Tk_GCForColor(textPtr->fgColor, Tk_WindowId(textPtr->tkwin));
	XFillRectangle(display, pixmap, gc,
	    dInfoPtr->x - dInfoPtr->curXPixelOffset,
	    y + dlPtr->height - 1,
	    (unsigned int) (dInfoPtr->maxX - dInfoPtr->x),
	    (unsigned int) 1);
	dlPtr->flags |= DLINE_FOLDED;
    } else {
	dlPtr->flags &= ~DLINE_FOLDED;
    }
#endif
    ...
    ClientData clientData)	/* Information about widget. */
{
    ...
    if (dInfoPtr->flags & REDRAW_PENDING) {
dbwin("AsyncUpdateLineMetrics %s REDRAW_PENDING\n", Tk_PathName(textPtr->tkwin));
#ifdef STEXT_DIFF
	if (--textPtr->refCount == 0) {
	    ckfree((char *) textPtr);
	} else {
	    dInfoPtr->flags |= DINFO_UPDATE_METRICS;
	}
#else
...
/*dbwin("AsyncUpdateLineMetrics %s %d %d\n", Tk_PathName(textPtr->tkwin), lineNum, dInfoPtr->lastMetricUpdateLine); */
	 ...
	 * above).
	 */
dbwin("AsyncUpdateLineMetrics %s ended at lineNum %d\n", Tk_PathName(textPtr->tkwin), lineNum);
...
/*dbwin("TextInvalidateLineMetrics %s linePtr %p lineCount %d", Tk_PathName(textPtr->tkwin), linePtr, lineCount); */

    if (linePtr != NULL) {
#ifdef STEXT_DIFF
	int fakeLine = BTREE_NUMLINES(textPtr);
	int lastLine = PEER_NUMLINES(textPtr);
	int counter;

	if (lastLine == fakeLine)
	    --lastLine;

	fromLine = PEER_LINESTO(textPtr, linePtr);

	if (fromLine + lineCount < 0)
	    return;
	...
	if (fromLine + lineCount > lastLine) {
	    lineCount -= fromLine + lineCount - lastLine;
	}
...
#ifdef STEXT_LINE_VISIBLE
	    if (/*GetLineVisible(textPtr, endOfLastLine.linePtr) &&*/
		    !TkTextIsElided(textPtr, &endOfLastLine, NULL)) {
#else
...
#ifdef STEXT_LINE_VISIBLE
	    if (/*GetLineVisible(textPtr, idx.linePtr) &&*/
		    !TkTextIsElided(textPtr, &idx, NULL)) {
#else
    ...
    ClientData clientData)	/* Information about widget. */
{
...
#ifdef STEXT_DIFF
    /* Generate an event when the insertion cursor moves. */
    TkTextMarkSegToIndex(textPtr, textPtr->insertMarkPtr, &index);
    linesTo = PEER_LINESTO(textPtr, index.linePtr);
    if (linesTo != dInfoPtr->insertLinesTo ||
	    index.byteIndex != dInfoPtr->insertByteIndex) {
	XEvent event;

	dInfoPtr->insertLinesTo = linesTo;
	dInfoPtr->insertByteIndex = index.byteIndex;
	...
	((XVirtualEvent *) &event)->name = Tk_GetUid("TextInsertionCursorMoved");
...
#ifdef STEXT_MARGINS
	    /* If a line overlaps the top/bottom... */
	    dlPtr->flags |= DLINE_MARGINS;
#endif
	    continue;
	}
...
#ifdef STEXT_MARGINS
		dlPtr2->flags |= OLD_Y_INVALID | DLINE_MARGINS;
#else
...
#ifdef STEXT_MARGINS
	{
	int left, right, marginLeft, marginRight;
	TkTextGetMarginIndents(textPtr, &marginLeft, &marginRight);
	borders = textPtr->borderWidth + textPtr->highlightWidth;
	left = marginLeft ? borders : dInfoPtr->x;
	right = marginRight ? Tk_Width(textPtr->tkwin) - borders : dInfoPtr->maxX;
	if (left >= right)
	    right = left + 1;
	...
	if (textPtr->padX > 0) {
	    ...
	    TkTextGetMarginIndents(textPtr, &left, &right);
	    Tk_Fill3DRectangle(textPtr->tkwin, Tk_WindowId(textPtr->tkwin),
		textPtr->border, borders + left, borders + textPtr->padY,
		textPtr->padX,
		Tk_Height(textPtr->tkwin) - 2*borders -2*textPtr->padY,
		0, TK_RELIEF_FLAT);
	    Tk_Fill3DRectangle(textPtr->tkwin, Tk_WindowId(textPtr->tkwin),
		    textPtr->border,
		    Tk_Width(textPtr->tkwin) - borders - right - textPtr->padX,
...
#ifdef STEXT_MARGINS
	    if (dlPtr->flags & DLINE_MARGINS) {
/*dbwin("DLINE_MARGINS %s %d", Tk_PathName(textPtr->tkwin), BTREE_LINESTO(NULL,
				dlPtr->index.linePtr) + 1);*/
		TkTextDrawMargins(textPtr, dlPtr->index.linePtr, pixmap,
			dInfoPtr->copyGC, dlPtr->y, dlPtr->height,
			dlPtr->baseline, dlPtr->flags);
		dlPtr->flags &= ~DLINE_MARGINS;
#ifdef STEXT_LINE_NUMBER
		if (TkTextMarginBounds(textPtr, textPtr->marginN, NULL, NULL)) {
		    /* Remember the line number, so we can redisplay this line if the line
		    * number changes. */
		    if (textPtr->lineNumbers == LINE_NUMBERS_DOCUMENT)
			dlPtr->lineNumber = BTREE_LINESTO(NULL,
				dlPtr->index.linePtr) + 1;
		    else if (textPtr->lineNumbers == LINE_NUMBERS_RELATIVE)
			dlPtr->lineNumber = PEER_LINESTO(textPtr,
				dlPtr->index.linePtr) + 1;
		}
#endif
#ifdef STEXT_FOLDING
		if (TkTextMarginBounds(textPtr, textPtr->marginF, NULL, NULL)) {
		    /* Remember the fold mark, so we can redisplay this line
		     * if the fold mark changes. */
		    dlPtr->foldMark = GetFoldMark(textPtr,
			    dlPtr->index.linePtr, dlPtr->flags);

		    /* Remember whether the fold mark is drawn with the
		     * active color, so we can redisplay this line if the
		     * highlight status changes. */
		    if (GetLineFoldHighlight(textPtr, dlPtr->index.linePtr))
			dlPtr->flags |= DLINE_FOLD_HIGHLIGHT;
		    else
			dlPtr->flags &= ~DLINE_FOLD_HIGHLIGHT;
		}
#endif
...
#ifdef STEXT_MARGINS
	borders = textPtr->highlightWidth + textPtr->borderWidth;
	Tk_Fill3DRectangle(textPtr->tkwin, Tk_WindowId(textPtr->tkwin),
	    textPtr->border, borders,
	    bottomY, Tk_Width(textPtr->tkwin) - borders * 2,
...
#ifdef STEXT_DIFF
    if (dInfoPtr->flags & DINFO_UPDATE_METRICS) {
	dInfoPtr->flags &= ~DINFO_UPDATE_METRICS;
	if (dInfoPtr->lineUpdateTimer == NULL) {
dbwin("DINFO_UPDATE_METRICS %s", Tk_PathName(textPtr->tkwin));
	    textPtr->refCount++;
	    dInfoPtr->lineUpdateTimer = Tcl_CreateTimerHandler(1,
		    AsyncUpdateLineMetrics, (ClientData) textPtr);
	}
...

void
TkTextEventuallyRelayoutWindow(
    TkText *textPtr)		/* Widget record for text widget. */
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;

    dInfoPtr->flags |= DINFO_OUT_OF_DATE | DINFO_RELAYOUT_WINDOW;
    if (!(dInfoPtr->flags & REDRAW_PENDING)) {
	dInfoPtr->flags |= REDRAW_PENDING;
	Tcl_DoWhenIdle(DisplayText, (ClientData) textPtr);
    }
...

void
TkTextEventuallyRedrawMarginLine(
    TkText *textPtr,		/* Widget record for text widget. */
    TkTextLine *linePtr)
{
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    DLine *dlPtr = dInfoPtr->dLinePtr;

    /* FIXME: chould be 1000s of hidden/elided lines. */
    while (dlPtr != NULL) {
	if (dlPtr->index.linePtr == linePtr)
	    dlPtr->flags |= DLINE_MARGINS;
	dlPtr = dlPtr->nextPtr;
    }
    if (!(dInfoPtr->flags & REDRAW_PENDING)) {
    ...
    TkRegion region)		/* Region of area to redraw. */
{
    ...
    for (dlPtr = dInfoPtr->dLinePtr; dlPtr != NULL;
	    dlPtr = dlPtr->nextPtr) {
#ifdef STEXT_MARGINS
	if (TkRectInRegion(region, rect.x, dlPtr->y,
...
#ifdef STEXT_MARGINS
    TkTextGetMarginIndents(textPtr, &inset[0], &inset[2]);
    inset[0] += textPtr->borderWidth + textPtr->highlightWidth;
    inset[2] += textPtr->borderWidth + textPtr->highlightWidth;
    inset[1] = inset[3] = textPtr->borderWidth + textPtr->highlightWidth;
    if ((rect.x < (inset[0] + textPtr->padX))
	    || (rect.y < (inset[1] + textPtr->padY))
	    || ((int) (rect.x + rect.width) > (Tk_Width(textPtr->tkwin)
		    - inset[2] - textPtr->padX))
	    || (maxY > (Tk_Height(textPtr->tkwin) - inset[3] - textPtr->padY))) {
	dInfoPtr->flags |= REDRAW_BORDERS;
    }
#else /* STEXT_MARGINS */
...
#if 0
{
    char pos1[TK_POS_CHARS], pos2[TK_POS_CHARS];
    if (index1Ptr) TkTextPrintIndex(textPtr, index1Ptr, pos1);
    else strcpy(pos1, "NULL");
    if (index2Ptr) TkTextPrintIndex(textPtr, index2Ptr, pos2);
    else strcpy(pos2, "NULL");
dbwin("TextRedrawTag %s index1Ptr %s index2Ptr %s tag %s %d", Tk_PathName(textPtr->tkwin), pos1, pos2, tagPtr->name, withTag);
}
#endif
...
#ifdef STEXT_STYLE_HACK
    /* If this tag is one of the lexer's tags, redraw everything. */
    if (textPtr->sharedTextPtr->lexer != NULL) {
	if (Lexer_OwnsTag(textPtr->sharedTextPtr->lexer, tagPtr)) {
				 ...
				 * are to be off the top of the screen. */
{
    ...
    if (PEER_NUMLINESVIS(textPtr) == 0) {
	goto scheduleUpdate;
    }
    linePtr = FindNearestVisibleLine(textPtr, indexPtr->linePtr);
				 ...
				 * window. */
{
    ...
    /* Empty string is returned if nothing is at the point. */
    Tcl_ResetResult(interp);

    if (y < dInfoPtr->y) return;
    if (y >= dInfoPtr->maxY) return;
    if (x >= Tk_Width(textPtr->tkwin) - borders) return;
    if (x < borders) return;
    ...
    for (dlPtr = dInfoPtr->dLinePtr;
	    (dlPtr != NULL) && (y >= dlPtr->y + dlPtr->height);
	    dlPtr = dlPtr->nextPtr) {
	/* nothing */
    }

    /* Y-coordinate is below any lines of text. */
    if (dlPtr == NULL) {
	lineNumber = 0;
    } else {
	lineNumber = TkBTreeLinesTo(textPtr, dlPtr->index.linePtr) + 1;
    }

#ifdef STEXT_MARGINS
    marginPtr = TkTextMarginAtX(textPtr, x);
    if (marginPtr != NULL) {
	sprintf(result, "margin %s", TkTextMarginName(textPtr, marginPtr));
	if (lineNumber > 0) {
	    sprintf(result + strlen(result), " line %d", lineNumber);
	}
    ...
    /* Not in a margin and below any display lines or over the left -padx. */
    if (lineNumber <= 0 || x < dInfoPtr->x)
	return;
    ...
    /* Index at the start of the display line. */
    index = dlPtr->index;
    ...
    for (chunkPtr = dlPtr->chunkPtr;
	    (chunkPtr != NULL) && (x >= chunkPtr->x + chunkPtr->width);
	    index.byteIndex += chunkPtr->numBytes,
	    chunkPtr = chunkPtr->nextPtr) {
	/* nothing */
    }

    /* Over a line, but outside any display chunks. */
    if (chunkPtr == NULL) {
	return;
    }
	...
	index.byteIndex += (*chunkPtr->measureProc)(chunkPtr, x);
    }

    sprintf(result, "index ");
    TkTextPrintIndex(textPtr, &index, result + strlen(result));
    Tcl_SetResult(interp, result, TCL_VOLATILE);
}
#endif /* STEXT_DIFF */

...
#if !TK_LAYOUT_WITH_BASE_CHUNKS
#ifdef STEXT_DLINE_CACHE
    if (textPtr->dInfoPtr->dCharInfoFreePtr != NULL) {
	ciPtr = textPtr->dInfoPtr->dCharInfoFreePtr;
	textPtr->dInfoPtr->dCharInfoFreePtr = ciPtr->nextPtr;
    } else {
	ciPtr = (CharInfo *) ckalloc(sizeof(CharInfo));
    }
    ciPtr->byteOffset = Tcl_DStringLength((Tcl_DString *) chunkPtr->clientData);
    Tcl_DStringAppend((Tcl_DString *) chunkPtr->clientData, p, bytesThatFit);
    ciPtr->chars = Tcl_DStringValue((Tcl_DString *) chunkPtr->clientData) + ciPtr->byteOffset;
    chunkPtr->clientData = (ClientData) ciPtr;
#else
    ciPtr = (CharInfo *)
...
#endif /* TK_LAYOUT_WITH_BASE_CHUNKS */
#ifdef STEXT_DRAW_EOL
    ciPtr->newline = (p[bytesThatFit - 1] == '\n') && textPtr->showEOL;
    if (ciPtr->newline)
	chunkPtr->width += 8;
#endif
...
#ifdef STEXT_DRAW_EOL
    /* Draw the EOL indicator */
    if (ciPtr->newline && !sValuePtr->elide) {
	int numBytes = ciPtr->numBytes - offsetBytes;
	CONST char *string = ciPtr->chars + offsetBytes;
	int width = 0;
	int ymid = y + height / 2;
	if (numBytes)
	    Tk_MeasureChars(sValuePtr->tkfont, string, numBytes, -1, 0, &width);
	XDrawLine(display, dst, textPtr->wsGC,
	    offsetX + width + 1, ymid, offsetX + width + 8, ymid);
	XDrawLine(display, dst, textPtr->wsGC,
	    offsetX + width + 8, ymid, offsetX + width + 8, ymid + 4);
    }
#endif /* STEXT_DRAW_EOL */
...
#ifdef STEXT_DRAW_TAB
	/* Draw tabs */
	if (textPtr->showTabs && chunkPtr->nextPtr) {
	    int width;
	    int diff = x - chunkPtr->x;
	    int ymid = y + height / 2;
	    int xmin, ymin, xmax, ymax, ydiff, xhead;
	    Tk_MeasureChars(sValuePtr->tkfont, string, numBytes, -1, 0, &width);
	    xmin = offsetX + width + 1;
	    ymin = y + 4;
	    xmax = (offsetX + width) + (chunkPtr->nextPtr->x + diff) - (x + width) - 1;
	    ymax = y + height - 4;
	    ydiff = (ymax - ymin) / 2;
	    xhead = xmax - 1 - ydiff;
	    if (xhead <= xmin)
		...
		xmin, ymid, xmax, ymid);
	    XDrawLine(display, dst, textPtr->wsGC,
		xhead, ymid - ydiff, xmax - 1, ymid);
	    XDrawLine(display, dst, textPtr->wsGC,
		xhead, ymid + ydiff, xmax - 1, ymid);
	}
#endif /* STEXT_DRAW_TAB */
...
#ifdef STEXT_DLINE_CACHE
	ciPtr->nextPtr = textPtr->dInfoPtr->dCharInfoFreePtr;
	textPtr->dInfoPtr->dCharInfoFreePtr = ciPtr;
#else
	ckfree((char *) ciPtr);
#endif
	chunkPtr->clientData = NULL;
...
#ifdef STEXT_DIFF
	    int tabWidth = Tk_TextWidth(textPtr->tkfont, "0", 1) * textPtr->tabWidth;
#else
...
#ifdef STEXT_DIFF
	tabWidth = Tk_TextWidth(textPtr->tkfont, "0", 1) * textPtr->tabWidth;
#else
...
#ifdef STEXT_DIFF
    tabWidth = Tk_TextWidth(tkfont, "0", 1) * textPtr->tabWidth;
#else
    ...
    bool visible)
{
    ...
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    bool wasVisible = (linePtr->peerData[ref].flags & LINE_FLAG_HIDDEN) == 0;
    bool inBounds = true;
    int linesTo;

    if (visible != wasVisible) {
	linesTo = BTREE_LINESTO(textPtr, linePtr);
	if (textPtr->start != NULL) {
	    if (linesTo < BTREE_LINESTO(textPtr, textPtr->start))
		inBounds = false;
	}
	if (textPtr->end != NULL) {
	    if (linesTo >= BTREE_LINESTO(textPtr, textPtr->end))
		inBounds = false;
	}
	...
	if (inBounds) {
	    ...
	    PEER_BYTEINDEX(textPtr, PEER_LINESTO(textPtr, linePtr), 0, &index1);
	    /* FIXME: back 1 char? */
	    PEER_BYTEINDEX(textPtr, PEER_LINESTO(textPtr, linePtr) + 1, 0, &index2);
	    PEER_TEXTCHANGED(textPtr, &index1, &index2);

	    if (visible) {
		TkTextInvalidateLineMetrics(NULL, textPtr,
			linePtr, 0, TK_TEXT_INVALIDATE_ONLY);
		if (TkBTreeLinePixelCount(textPtr, linePtr) != textPtr->charHeight) {
		    TkBTreeAdjustPixelHeight(textPtr, linePtr, textPtr->charHeight, 0);
		}
	    } else {
		TkBTreeLinePixelEpoch(textPtr, linePtr)
			= textPtr->dInfoPtr->lineMetricUpdateEpoch;
	...
	if (!visible) {
	    linePtr->peerData[ref].flags |= LINE_FLAG_HIDDEN;
	} else {
	    linePtr->peerData[ref].flags &= ~LINE_FLAG_HIDDEN;
	}

	TkBTreeToggleLineVisible(textPtr, linePtr);
...
{
    int ref = textPtr->pixelReference;
    return (linePtr->peerData[ref].flags & LINE_FLAG_HIDDEN) == 0;
}

#endif /* STEXT_LINE_VISBIBLE */
    ...
    bool folded)
{
    int ref = textPtr->pixelReference;
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    bool wasFolded = (linePtr->peerData[ref].flags & LINE_FLAG_FOLDED) != 0;

    if (folded != wasFolded) {
	if (folded)
	    linePtr->peerData[ref].flags |= LINE_FLAG_FOLDED;
	else
	    linePtr->peerData[ref].flags &= ~LINE_FLAG_FOLDED;
...
GetLineFoldable(
    ...
    TkTextLine *linePtr)
{
    return (linePtr->level & SC_FOLDLEVELHEADERFLAG) != 0;
}
...
{
    int ref = textPtr->pixelReference;
    return (linePtr->peerData[ref].flags & LINE_FLAG_FOLDED) != 0;
}

void
SetFoldHighlightLine(
    TkText *textPtr,
    TkTextLine *linePtr)
{
    ...
    int ref = textPtr->pixelReference;
    TextDInfo *dInfoPtr = textPtr->dInfoPtr;
    TkTextLine *oldParent = NULL, *oldLast = NULL;
    TkTextLine *parent = NULL, *last = NULL;
    TkTextLine *temp;

    if (textPtr->foldHighlightLinePtr != NULL) {
	oldParent = textPtr->foldHighlightLinePtr;
	if (!GetLineFoldable(sharedPtr, oldParent)) {
	    temp = GetFoldParent(sharedPtr, oldParent);
	    if (temp != NULL)
		oldParent = temp;
	}
	oldLast = GetLastChild(sharedPtr, oldParent);
    }

    if (linePtr != NULL) {
	parent = linePtr;
	if (!GetLineFoldable(sharedPtr, parent)) {
	    temp = GetFoldParent(sharedPtr, parent);
	    if (temp != NULL)
		parent = temp;
	}
	last = GetLastChild(sharedPtr, parent);
    }
    if (parent == oldParent && last == oldLast)
	return;

    if (oldParent != NULL) {
	temp = oldParent;
	while (temp != NULL) {
	    temp->peerData[ref].flags &= ~LINE_FLAG_HIGHLIGHT;
	    if (temp == oldLast)
		break;
	    temp = PEER_NEXTLINE(textPtr, temp);
	}
    }
    ...
    textPtr->foldHighlightLinePtr = linePtr;
...
bool
GetLineFoldHighlight(
    TkText *textPtr,
    TkTextLine *linePtr)
{
    int ref = textPtr->pixelReference;
    return (linePtr->peerData[ref].flags & LINE_FLAG_HIGHLIGHT) != 0;
}
...
SetLineFoldLevel(
    ...
    int level)
{
    /* Prevent depth from being negative. */
    if (depth < 0)
	depth = 0;
    depth &= SC_FOLDLEVELNUMBERMASK;
    /* Prevent level flags being in the depth bits. */
    level &= ~SC_FOLDLEVELNUMBERMASK;
    linePtr->level = depth | level;
}
...
GetLineFoldDepth(
    ...
    TkTextLine *linePtr)
{
    return linePtr->level & SC_FOLDLEVELNUMBERMASK;
}
