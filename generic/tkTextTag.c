...
/* Create a custom option like TK_OPTION_STRING_TABLE, but supporting
 * TK_OPTION_NULL_OK. 8.4.12 does not support "" keys, while 8.4.13+ does. */

typedef struct StringTableCOClientData {
    const char *optionName;
    const char **tablePtr;
    int emptyVal;
} StringTableCOClientData;
...
static int
StringTableCO_Set(clientData, interp, tkwin, value, recordPtr, internalOffset,
...
{
    StringTableCOClientData *cd = clientData;
    int index, *internalPtr;
	...
	*value = NULL;
	...
	if (Tcl_GetIndexFromObj(interp, *value, cd->tablePtr,
		cd->optionName+1, 0, &index) != TCL_OK) {
	    return TCL_ERROR;
	}
    }

    if (internalPtr != NULL) {
	*((int *) oldInternalPtr) = *internalPtr;
	*internalPtr = index;
    }
    return TCL_OK;
}
...
static Tcl_Obj *
StringTableCO_Get(clientData, tkwin, recordPtr, internalOffset)
    ClientData clientData;
    Tk_Window tkwin;
				 ...
				 * line value. */
{
    ...
    if (*internalPtr == cd->emptyVal)
	return Tcl_NewObj();
    return Tcl_NewStringObj(cd->tablePtr[*internalPtr], -1);
}

static void
StringTableCO_Restore(clientData, tkwin, internalPtr, oldInternalPtr)
    ClientData clientData;
    Tk_Window tkwin;
...
static Tk_ObjCustomOption stringTableCO = {
    "stringtable",		/* name */
    StringTableCO_Set,		/* setProc */
    StringTableCO_Get,		/* getProc */
    StringTableCO_Restore,	/* restoreProc */
    NULL,			/* freeProc */
    0
};
...
static const Tk_OptionSpec tagOptionSpecs[] = {
...
#ifdef STEXT_DIFF
    {TK_OPTION_CUSTOM, "-tabstyle", NULL, NULL,
	...
	TK_OPTION_NULL_OK, 0, 0},
#ifdef STEXT_DIFF
    {TK_OPTION_CUSTOM, "-wrap", NULL, NULL,
				 ...
				 * objv[1] is "tag". */
{
...
#ifdef STEXT_DIFF
		/* Calling ".t tag remove $tag 1.0 end" causes the line metrics
		 * calculator to spring into action for the *entire* document
		 * if the tag affects line geometry.
		 * This is a bad thing if zero or only a few lines are
		 * actually tagged. */
		if (optionIndex == TAG_REMOVE) {
		    TkTextSearch search;
		    TkTextIndex first = index1;
		    int tagOn = 0;

		    TkBTreeStartSearch(&first, &index2, tagPtr, &search);
			...
			if (tagOn) {
			    TkTextRedrawTag(textPtr->sharedTextPtr, NULL,
				    &first,
				    &search.curIndex, /* FIXME: back 1 char? */
				    tagPtr, 1);
			    tagOn = 0;
			} else {
			    tagOn = 1;
			    first = search.curIndex;
			}
		    }
		    if (tagOn) {
			TkTextRedrawTag(textPtr->sharedTextPtr, NULL, &first,
				&index2,
				tagPtr, 1);
		    }
		} else
	...
	for (i = 3; i < objc; i++) {
	    ...
	    if ((textPtr->sharedTextPtr->lexer != NULL) &&
		    Lexer_OwnsTag(textPtr->sharedTextPtr->lexer, tagPtr)) {
		continue;
	    }
#endif
	    if (tagPtr->affectsDisplay) {
...
#ifdef STEXT_DIFF
	    arrayPtr = TkBTreeGetTags(&index1, textPtr, &tagInfo);
	    arraySize = tagInfo.numTags;
#else
...
#ifdef STEXT_DIFF
    {
	Tk_OptionSpec *specPtr = tagOptionSpecs;
	while (specPtr->type != TK_OPTION_END) {
	    if (!strcmp(specPtr->optionName, "-tabstyle") &&
		    (specPtr->clientData == NULL)) {
		Tk_ObjCustomOption *co = (void *) ckalloc(sizeof(*co));
		StringTableCOClientData *cd = (void *) ckalloc(sizeof(*cd));
		cd->optionName = specPtr->optionName;
		cd->tablePtr = tabStyleStrings;
		cd->emptyVal = TK_TEXT_TABSTYLE_NONE;
		*co = stringTableCO;
		co->clientData = cd;
		specPtr->clientData = co;
	    }
	    if (!strcmp(specPtr->optionName, "-wrap") &&
		    (specPtr->clientData == NULL)) {
		Tk_ObjCustomOption *co = (void *) ckalloc(sizeof(*co));
		StringTableCOClientData *cd = (void *) ckalloc(sizeof(*cd));
		cd->optionName = specPtr->optionName;
		cd->tablePtr = wrapStrings;
		cd->emptyVal = TEXT_WRAPMODE_NULL;
		*co = stringTableCO;
		co->clientData = cd;
		specPtr->clientData = co;
	    }
...
#ifdef STEXT_DIFF
	    TkTextTagInfo tagInfo;
	    newArrayPtr = TkBTreeGetTags(&index, textPtr, &tagInfo);
	    numNewTags = tagInfo.numTags;
	    if (tagInfo.arraySize > LOTSA_TAGS) {
		ckfree((char *) tagInfo.counts);
	    } else if (numNewTags > 0) {
		newArrayPtr = (TkTextTag **) ckalloc(sizeof(TkTextTag *) * numNewTags);
		memcpy(newArrayPtr, tagInfo.tagPtrs, sizeof(TkTextTag *) * numNewTags);
	    }
#else
